...
let privateFeedback = JSON.parse(localStorage.getItem('newsPrivateFeedback')) || [];
...
function initPrivateFeedback() {
    renderPrivateFeedback();
    const form = document.getElementById('newsFeedbackForm');
    if (!form) return;
    form.addEventListener('submit', function(event) {
        ...
        privateFeedback.unshift(entry);
        localStorage.setItem('newsPrivateFeedback', JSON.stringify(privateFeedback));
        form.reset();
        renderPrivateFeedback();
        showNotification('Заявка отправлена. Она сохранена приватно и не видна другим пользователям.', 'success');
    });
}
...
function renderPrivateFeedback() {
    ...
    if (!privateFeedback.length) {
        container.innerHTML = '<div class="private-feedback-item"><h4>Приватный канал связи</h4><p>Здесь отображаются только ваши обращения из этого браузера. Другие пользователи их не видят.</p></div>';
        return;
    }
    container.innerHTML = privateFeedback.slice(0, 5).map(item => `
        <article class="private-feedback-item">
            <h4>${item.topic}</h4>
            <div class="news-meta">${item.name} · ${new Date(item.createdAt).toLocaleString('ru-RU')}</div>
            <p>${item.message}</p>
            <div class="news-meta">Ответ будет направлен на: ${item.email}</div>
        </article>
    `).join('');
}
